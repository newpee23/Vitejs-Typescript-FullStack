...
import { Async_addCustommer, loadAsync_Amphures, loadAsync_Districts, loadAsync_Geographies, loadAsync_Provinces } from '../store/slices/formInput';
...
export const Form: FC = () => {
  ...
  const { error } = useAppSelector((state) => state?.forminput);
  // const { loading } = useAppSelector((state) => state?.forminput);
  ...
  const onSubmitFrom = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const addDataCustommer = async () => {
      dispatch(Async_addCustommer(data));
    };
    addDataCustommer();
                  ...
                  <form onSubmit={onSubmitFrom}>
                    ...
                    <div className="col-span-5 mt-3 text-right">
                      <div className='flex justify-between items-center'>
                        {error == 'มีข้อมูล Username และ Password แล้วในระบบ' ? (
